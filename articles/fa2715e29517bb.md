...
class updateAction
{
    ...
    public function __invoke(array $validatedRequests): UserContract|bool
    {
        // バグ対策の簡易アサーション
        assert(
            isset(
        ...
        try {
            $user = $this->db->transaction(function () use ($validatedRequests) { // トランザクションを貼る
...
今回は，モジュラモノリスなど，**ガッツリパッケージそれぞれを疎結合にしてインターフェースに依存させるようなケース**を想定しているので，なんちゃってクリーンアーキテクチャを参考にしていますが Eloquent Model と Entity の橋渡し役として Repository 層を作っています．
...
* *`UserRepositoryContract::class`* の
    ...
    * `setBirthday()`
    * `save()`
* *`ExceptionHandler::class`* の `report()`
...
{
    $exception = \Mockery::mock(ExceptionHandler::class);
...
`andReturnUsing()` の引数に渡しているクロージャの引数では，モックしているメソッドが実際にコールされるときに渡された値を受け取ります．
...
ここで，**チェーンされているメソッド郡のモック方法**も紹介します．
このとき，
...
/**
 * @var ExceptionHandler&MockInterface&mixed
 */
private ExceptionHandler $exception;
...
protected function setUp(): void
{
    ...
    $this->exception = Mockery::mock(ExceptionHandler::class);
